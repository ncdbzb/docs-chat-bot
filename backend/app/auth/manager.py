...
class UserManager(IntegerIDMixin, BaseUserManager[AuthUser, int]):
    ...
    async def on_after_forgot_password(
        ...
        logger.info(f"User {user.email} has forgot his password")
        token_data = {
            "sub": str(user.id),
    ...
    async def create(
        ...
        user_dict = (
            ...
            if safe else user_create.create_update_dict_superuser()
        )

        password = user_dict.pop("password")
        user_dict.pop("confirmation_password", None)

        if user_dict["email"] == settings.ADMIN_EMAIL:
            if password != settings.ADMIN_PASSWORD:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Forbidden: wrong admin credentials"
                )
